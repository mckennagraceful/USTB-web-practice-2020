<template>
	<view class="register">
		...
		<view class="content">
			...
			<!-- 主体 -->
			<view class="main">
				<wInput v-model="user.mobile" type="text" maxlength="11" placeholder="手机号"></wInput>
				<wInput v-model="user.password" type="password" maxlength="11" placeholder="登录密码" isShowPass></wInput>
				<wInput v-model="verCode" type="text" maxlength="6" placeholder="验证码" isShowCode ref="runCode" @setCode="getVerCode()"></wInput>
...
<script>
	...
	export default {
		...
		methods: {
			...
			createRandomCodeString() {
				let str = '0123456789abcdefghjklmnpqrstuvwxyz';
				let res = '';
				for (let i = 0; i < 6; i++) {
					//随机产生字符串的下标
				...
				console.log("本次生成的验证码是：" + res);
				return res;
			},
			getVerCode() {
				//获取验证码
				if (_this.user.mobile.length != 11) {
					uni.showToast({
						icon: 'none',
						position: 'bottom',
						title: '手机号不正确'
				...
				console.log("真正去发送验证码...");
				let randomCodeStr = this.createRandomCodeString(); //生成验证码
				// promise方式
				...
				setTimeout(function() {
					_this.$refs.runCode.$emit('runCode', 60);
				}, 3000)
				uni.showToast({
					icon: 'none',
					position: 'bottom',
